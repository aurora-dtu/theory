...
-/
def ℬ_b_exists_decomposition (hsb : B ∈ ℬ{b}) (hb : b.Finite) :
    ∃ q : Set (Set H),
      B = ⋃ a ∈ q, A{a,b} ∧ Set.PairwiseDisjoint q (A{·,b}) ∧ q.Finite ∧ ∀ a ∈ q, a ⊆ b := by
  ...
  | union s t hs ht ihs iht =>
    ...
    · simp_all
      rintro a (ha | ha)
      · exact ihs.right.right _ ha
      · exact iht.right.right _ ha

/-- **Lemma 3.** (i) -/
theorem A_ab_subset_iff (hab : a ⊆ b) (hab' : a' ⊆ b') :
    A{a,b} ⊆ A{a',b'} ↔ a' ⊆ a ∧ b' \ a' ⊆ b \ a := sorry
