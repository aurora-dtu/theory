...
-- inductive Thingy : Set (Set H) → Set (Set (Set (Set H))) where
--   | base (b : Set H) (hb : b.Finite) : Thingy B{b} {B{b}}
...
def join (S : Set (Set (Set H))) : Set (Set H) := ⋃₀ S

def ℬ_b_gen {B : Set (Set H)} (hB : B ∈ ℬ{Set.univ}) : Set H := ℬ_b_mem_iff_exists.mp hB |>.choose
def ℬ_b_gen_spec {B : Set (Set H)} (hB : B ∈ ℬ{Set.univ}) :
...
@[simp]
theorem b_B_Finite (B : ℬ{(Set.univ : Set H)}) : b{B}.Finite := b_B_gen_spec B |>.left
omit [DecidableEq H] in
@[simp]
...
noncomputable def IE (μ : B_b_fin (H:=H) → ENNReal) (a b : Set H) (hb : b.Finite) : ℝ :=
  ∑' c : {c | a ⊆ c ∧ c ⊆ b},
...
omit [DecidableEq H] in
theorem bI_finite (I : Finset (Set (Set H))) (hI : I.toSet ⊆ ℬ{Set.univ}) : bI{I, hI}.Finite := by
  refine Set.finite_iUnion ?_
  exact fun i ↦ b_B_Finite ⟨↑i, hI (Subtype.prop i)⟩

omit [DecidableEq H] in
theorem bI_mem (I : Finset (Set (Set H))) (hI : I.toSet ⊆ ℬ{Set.univ}) :
    ∀ i ∈ I, i ∈ ℬ{bI{I, hI}} := by
  intro i hi
...
omit [DecidableEq H] in
theorem bI_subset (I : Finset (Set (Set H))) (hI : I.toSet ⊆ ℬ{Set.univ}) :
...
theorem 𝒜_B_b {b : Set H} (hb : b.Finite) : 𝒜 ⟨B{b}, by simp⟩ = B{b} := by
...
theorem crazy_B_b {b : Set H} (hb : b.Finite) :
...
theorem IE_indep_of_b (μ : B_b_fin (H:=H) → ENNReal) (a b₁ b₂ : Set H)
...
def condition (μ : B_b_fin (H:=H) → ENNReal) : Prop :=
...
noncomputable def the_thing (hb : b.Finite) (hab : a ⊆ b) (P : ℕ → Prop) :=
  ...
  Finset.nepowerset b.toFinset |>.filter (fun (c : Finset H) ↦ a ⊆ c.toSet ∧ P (c.val.card - aCard))

open scoped Classical in
/--
Extends a function from `{B{b} | b.Finite} → ENNReal` to `{A{a,b} | a ⊆ b ∧ b.Finite} → ENNReal`.
-/
noncomputable def extend_B_b_fin (μ : B_b_fin (H:=H) → ENNReal) :
    {(a, b) : Set H × Set H | a ⊆ b ∧ b.Finite} → ENNReal :=
  fun ⟨⟨a, b⟩, h⟩ ↦
...
-- noncomputable def extend_B_b_fin (μ : B_b_fin (H:=H) → ENNReal) :
--     ((fun ⟨a, b⟩ ↦ A{a,b}) '' {(a, b) : Set H × Set H | a ⊆ b ∧ b.Finite}) → ENNReal :=
--   fun ⟨A, h⟩ ↦ by
--     simp_all
...
ENNReal`.
-/
noncomputable def extend_A_ab (μ : {(a, b) : Set H × Set H | a ⊆ b ∧ b.Finite} → ENNReal) :
    {(b, B) : Set H × Set (Set H) | B ∈ ℬ{b} ∧ b.Finite} → ENNReal :=
  fun ⟨⟨b, B⟩, h⟩ ↦
    ∑' a : ℬ_b_cover h.left h.right, μ ⟨⟨a, b⟩, by
      obtain ⟨a, ha⟩ := a
      have := ℬ_b_cover_spec h.left h.right
      have := h.right
      simp_all⟩

/--
Extends a function from `{B | B ∈ ℬ{b} ∧ b.Finite} → ENNReal` to `{B : Set (Set H) | B ∈
ℬ{Set.univ}} → ENNReal`.
-/
noncomputable def extend_B (μ : {(b, B) : Set H × Set (Set H) | B ∈ ℬ{b} ∧ b.Finite} → ENNReal) :
    {B : Set (Set H) | B ∈ ℬ{Set.univ}} → ENNReal :=
  fun ⟨B, h⟩ ↦ μ ⟨⟨ℬ_b_gen h, B⟩, by simp [ℬ_b_gen_spec h]⟩

open scoped Classical in
noncomputable def extend_AddContent (μ : B_b_fin (H:=H) → ENNReal) :
    AddContent ℬ{(Set.univ : Set H)} where
  toFun := fun B ↦
...
open scoped Classical in
noncomputable def extend_AddContent' (μ : Set (Set H) → ENNReal) :
    AddContent ℬ{(Set.univ : Set H)} :=
  let μ' : B_b_fin (H:=H) → ENNReal := fun B ↦ μ B.val
  {
  toFun := fun B ↦
    if hB : B ∈ ℬ{Set.univ} then (μ' |> extend_B_b_fin |> extend_A_ab |> extend_B) ⟨B, hB⟩ else 0
  empty' := by
...
theorem extend_AddContent_IsSigmaSubadditive : (extend_AddContent (H:=H) μ).IsSigmaSubadditive := by
...
open scoped Classical in
def extend_measure_requirement (μ : B_b_fin (H:=H) → ENNReal) : Prop :=
  ∀ {a b : Set H} (hab : a ⊆ b) (h : B{b} ∈ B_b_fin (H:=H) ∧ b.Finite),
...
-/
noncomputable def extend_measure (μ : B_b_fin (H:=H) → ENNReal) (h : extend_measure_requirement μ) :
    Measure[ℬ.borel] (Set H)  :=
