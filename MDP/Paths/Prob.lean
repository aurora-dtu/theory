...
noncomputable def Prob (𝒮 : M.Scheduler') (π : M.Path) : ENNReal :=
  ∏ (i : Fin (∎|π| - 1)), M.P π[i] (𝒮 (π.take i)) π[i.succ]

@[simp]
theorem singleton_Prob (x : State) (𝒮 : M.Scheduler') : ({x} : M.Path).Prob 𝒮 = 1 := by
  simp [Prob]
  congr
...
theorem extend_Prob (s : M.succs_univ π.last) (𝒮 : M.Scheduler') :
