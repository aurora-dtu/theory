...
theorem EC_node_i_le_j_eq_top (h : i ≤ j) : 𝒜.EC 𝒜.cost 𝒮 (.node i j) n = ⊤ := by
...
@[simp]
theorem EC_step :
    𝒜.EC 𝒜.cost 𝒮 (.node i j) (n + 1) = 𝒜.EC 𝒜.cost 𝒮 (.node i (j + 1)) n := by
  simp [EC_succ]
  split_ifs
  · simp_all; rw [EC_node_i_le_j_eq_top (by omega)]
  · rw [tsum_eq_single ⟨.node i (j + 1), by simp_all [𝒜]⟩] <;> simp_all [𝒜]

@[simp]
theorem EC_node_i_j_n_eq_i_j_add_n :
    𝒜.EC 𝒜.cost 𝒮 (.node i j) n = 𝒜.EC 𝒜.cost 𝒮 (.node i (j + n)) 0 := by
  induction n generalizing i j <;> simp_all; split_ifs <;> first | rfl | omega

@[simp]
theorem EC_init_eq_EC_node :
    𝒜.EC 𝒜.cost 𝒮 .init (n + 1) = 𝒜.EC 𝒜.cost 𝒮 (.node (𝒮 {.init}) 0) n := by
  simp_all [EC_succ]
  split_ifs with h
