...
:= by
  simp only [ENNReal.tsum_add, ENNReal.tsum_mul_right, tsum_Prob_eq_one, one_mul]

theorem EC_succ [DecidableEq State] (𝒮 : M.Scheduler') :
    EC c 𝒮 s (n + 1) = ∑' s' : M.succs_univ s, M.P s (𝒮 {s}) s' * (c s + EC c 𝒮[s ↦ s'] s' n) := by
  simp [EC]
  rw [Path_eq.eq_succs_univ_biUnion', ENNReal.tsum_biUnion M.wtf_disjoint]
  congr! 2 with s'
...
theorem EC_succ' [DecidableEq State] (𝒮 : M.Scheduler') :
  ...
  simp [EC_succ, mul_add, ENNReal.tsum_add, ENNReal.tsum_mul_right]

theorem EC_eq (h : ∀ π ∈ Path[M,s,≤n], 𝒮 π = 𝒮' π) : EC c 𝒮 s n = EC c 𝒮' s n := by
  simp_all [EC, Path.ECost, Path.Prob]
theorem EC_le (h : ∀ π ∈ Path[M,s,≤n], 𝒮 π = 𝒮' π) : EC c 𝒮 s n ≤ EC c 𝒮' s n := le_of_eq <| EC_eq h
...
:= Function.Surjective.iInf_congr (·.bound) (by use ·.val; ext π hπ; simp_all)
  (fun _ ↦ EC_eq fun _ _ ↦ by simp [Scheduler'.specialize]; split_ifs <;> simp_all)
...
:= by
  ...
  apply iInf_le_of_le <| BScheduler'.mk' (M:=M) s (n+1) (fun ⟨π, hπ⟩ ↦
      if h : π.length ≤ 1 then M.default_act π.last
...
:= by
  ...
  · apply Function.Surjective.iInf_congr (·.bound) (by use ·.val; ext; simp_all [Scheduler'.bound])
    congr! 3; exact EC_eq fun _ _ ↦ by simp_all [Scheduler'.specialize]
...
theorem Φ𝒮_step_EC' (c : M.Costs) (𝒮 : M.Scheduler) :
  ...
  | succ n ih =>
    ...
    simp only [Path.instSingleton, Scheduler.toScheduler'_specialize, Scheduler.markovian,
      List.length_singleton, Nat.sub_self, not_lt_zero', List.getElem_singleton,
      List.getElem_cons_succ, forall_false, implies_true, Path.mk'_last, List.getElem_cons_zero,
