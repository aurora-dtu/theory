...
variable {State Act : Type*} [MeasurableSpace State]
...
omit [MeasurableSpace State]
variable [MeasurableSpace M.Path]
...
noncomputable def inducedMC' (M : MDP State Act) [MeasurableSpace M.Path] (𝒮 : 𝔖[M]) (ι : State) :
...
/-- The probability of a path `π` with scheduler `𝒮` is the measure of the cylinder set
  generated by `π` on the induced Markov Chain using `𝒮`. -/
